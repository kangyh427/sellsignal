// ============================================
// EnhancedMiniChart v3.1 - 캔들차트 + 기술적 분석 오버레이 + 터치/호버 툴팁
// 경로: src/components/EnhancedMiniChart.tsx
// 세션 62 hotfix: React Error #310 수정 (hooks 조건부 호출 위반)
//   - 모든 useMemo/useRef/useState/useCallback을
//     조건부 return(if !data return null) 이전으로 이동
//   - 차트 기간: 3개월 (90일) 대응
// ============================================
...
const EnhancedMiniChart: React.FC<EnhancedMiniChartProps> = ({
  ...
  // ★★★ 핵심 수정: 모든 hooks를 조건부 return 이전에 선언 ★★★
  const svgRef = useRef<SVGSVGElement>(null);
  const [hoverIdx, setHoverIdx] = useState<number | null>(null);

  // 안전한 데이터 (빈 배열이면 빈 값 사용)
  const safeData = data && data.length > 0 ? data : [];
  const hasData = safeData.length > 0;

  // ── 오버레이 계산 (모든 useMemo를 여기서 호출) ──
  ...
  const volProfile = useMemo(
    () => overlays.volumeProfile && safeData.length > 0 ? calcVolumeProfile(safeData) : null,
    [safeData, overlays.volumeProfile]
  );

  // 마우스/터치 핸들러 (useCallback도 조건부 return 전에)
  ...
  const getIdxFromEvent = useCallback((e: React.MouseEvent | React.TouchEvent) => {
    if (!svgRef.current || safeData.length === 0) return -1;
  ...
  }, [safeData.length, width, cW, pad.left, pad.right]);
  ...
  const barW = isWide
    ...
    : Math.max(1.5, cW / safeData.length - 1);
  ...
  const fs = {
    x: isWide ? 11 : isSmall ? 9 : 10,
    y: isWide ? 11 : isSmall ? 9 : 10,
    label: isWide ? 11 : isSmall ? 9 : 10,
  };
  ...
  // 매도선 렌더링
  const labelW = isWide ? 80 : isSmall ? 66 : 74;
  const labelH = 18;
  ...
  // X축 날짜 라벨 — 3개월 차트에 맞게 5~7개 라벨
  const xLabelCount = isSmall ? 4 : isWide ? 7 : 5;
  const xIndices: number[] = [];
  for (let i = 0; i < xLabelCount; i++) {
    xIndices.push(Math.round((safeData.length - 1) * i / (xLabelCount - 1)));
  }

  const buyLabelW = isWide ? 82 : isSmall ? 68 : 76;
  const buyLabelH = 20;
    ...
    <div style={{ position: 'relative', userSelect: 'none', touchAction: 'none' }}>
      ...
      {hoverIdx !== null && hoverCandle && (
        <div style={{
          ...
          {[
            ...
            <span key={label} style={{ display: 'inline-flex', gap: 2, alignItems: 'center' }}>
          ...
          {(() => {
            const prevClose = hoverIdx > 0 ? safeData[hoverIdx - 1].close : hoverCandle.open;
      ...
      >
        ...
        {volProfile && volProfile.map((zone, i) => {
          ...
          return (
            <rect key={`vp${i}`}
              x={2} y={y(zone.priceMax)}
              width={Math.max(1, barWidth)} height={Math.max(1, barHeight)}
        ...
        {/* X축 날짜 */}
        {xIndices.map((idx) => {
          if (idx >= safeData.length) return null;
          ...
          return (
            <g key={`c${i}`} opacity={hoverIdx !== null && !isHover ? 0.45 : 1}>
              <line x1={x(i) + barW / 2} y1={y(c.high)} x2={x(i) + barW / 2} y2={y(c.low)}
                stroke={col} strokeWidth={isWide ? 1 : 0.7} />
        ...
        {trendData && (
          <line
            x1={x(0) + barW / 2} y1={y(trendData.getY(0))}
        ...
        {hoverIdx !== null && hoverCandle && (
          <g>
            ...
            <rect
              ...
              width={isWide ? 82 : isSmall ? 62 : 72} height={18}
              fill="#334155" rx={3}
            />
            <text
