...
const MarketCycleWidget = ({ isMobile, isTablet, isPremium }: MarketCycleWidgetProps) => {
  const currentStage = 4;
  const stage = CYCLE_STAGES[currentStage - 1];

  // ★ 세션29: 텍스트 아이콘 + 밝은 배경으로 가시성 대폭 개선
  const indicators = [
    { textIcon: '₩', label: '한은금리', value: '3.5%', change: '▲', iconBg: 'rgba(239,68,68,0.2)', iconColor: '#f87171' },
    { textIcon: 'P', label: 'KOSPI PER', value: '11.8', change: '▼', iconBg: 'rgba(59,130,246,0.2)', iconColor: '#60a5fa' },
    { textIcon: '%', label: '국채3Y', value: '3.52%', change: '▲', iconBg: 'rgba(16,185,129,0.2)', iconColor: '#34d399' },
    { textIcon: '$', label: 'Fed금리', value: '4.5%', change: '→', iconBg: 'rgba(245,158,11,0.2)', iconColor: '#fbbf24' },
  ];
    ...
    <div style={{
      ...
      <div style={{ marginTop: isMobile ? '12px' : '16px' }}>
        <div style={{
          ...
          borderRadius: '12px', padding: isMobile ? '12px' : '14px', marginBottom: '10px',
        }}>
          ...
          <div style={{ fontSize: isMobile ? '13px' : '14px', color: '#cbd5e1', lineHeight: '1.5' }}>{stage.recommendation}</div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '4px' }}>
          {CYCLE_STAGES.map((s) => (
            <div key={s.num} style={{
              padding: isMobile ? '8px' : '7px 8px', borderRadius: '6px',
      ...
      <div style={{
        ...
        {indicators.map((ind, i) => (
          <div key={i} style={{
            ...
            padding: isMobile ? '10px 8px' : '10px 12px', textAlign: 'center',
          }}>
            <div style={{
              display: 'inline-flex', width: '30px', height: '30px',
              background: ind.iconBg, borderRadius: '8px',
              alignItems: 'center', justifyContent: 'center', marginBottom: '4px',
            }}>
              <span style={{ fontSize: '15px', fontWeight: '800', color: ind.iconColor }}>
                {ind.textIcon}
              </span>
            </div>
